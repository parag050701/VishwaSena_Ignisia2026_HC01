...
Produce EXACTLY this structure. Fill in real clinical content — no placeholders:

CLINICAL ASSESSMENT:
<write 3 sentences: (1) current clinical picture, (2) trajectory from timeline, (3) immediate survival risk>

DISEASE PROGRESSION TIMELINE SUMMARY:
<write 2-3 sentences: when deterioration began, what changed, current trajectory>
...
- <specific clinical concern> [<Source §Section>]
- <specific clinical concern> [<Source §Section>]
- <specific clinical concern> [<Source §Section>]

OUTLIER FLAGS (from Outlier Detection Module):
- <each held lab: name, value, Z-score, reason, required action> OR None detected

RECOMMENDED ACTIONS:
- <specific action> — IMMEDIATE
- <specific action> — URGENT
- <specific action> — ROUTINE

SHIFT HANDOVER BRIEF (30 seconds):
<write 2-3 plain English sentences for the incoming doctor>
...
SAFETY DISCLAIMER: This report is generated by an AI decision-support system (HC01). All outputs are decision-support only and do NOT constitute a clinical diagnosis. All flagged risks, drug interactions, and recommended actions must be verified and acted upon by a qualified clinician. Held diagnoses must not be confirmed until laboratory redraws are obtained and reviewed by clinical staff."""
    ...
    clean = re.sub(r'<think>[\s\S]*?</think>', '', full_text, flags=re.I).strip()

    # Find the LAST occurrence of CLINICAL ASSESSMENT: — the actual output, not template echo
    # Reasoning models often first echo the template then produce real content
    all_ca = list(re.finditer(r'CLINICAL ASSESSMENT\s*:', clean, re.I))
    if len(all_ca) > 1:
        structured = clean[all_ca[-1].start():].strip()
    elif all_ca:
        structured = clean[all_ca[0].start():].strip()
    else:
        structured = clean
